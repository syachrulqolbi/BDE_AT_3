select 
    ...
    avg(case when fl.has_availability = 't' then fl.price end) as avg_price_active,
    ...
    avg(case when fl.has_availability = 't' then fl.review_scores_rating end) as avg_review_scores_rating,
    ...
    avg(case when fl.has_availability = 't' then (30 - fl.availability_30) * fl.price end) as avg_estimated_revenue_active,
    
    -- Estimated Revenue per Host
    case 
        when count(distinct dh.host_id) = 0 then 0
        else avg((30 - fl.availability_30) * fl.price) / count(distinct dh.host_id)
    end as avg_estimated_revenue_per_host
